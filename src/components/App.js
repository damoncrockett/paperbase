...
import { makeMap, radarVertices, radarNormals } from "../utils/glyph";

import { makeGrid, makeHist, makeScatter } from "../utils/plot";
...
const expressivenessGroupArray = Array.from(
  new Set(data["expressivenessGroup"])
);
const expressivenessMap = makeMap(
  data,
  expressivenessGroupArray,
  "expressivenessGroup"
);

const isoGroupArray = Array.from(new Set(data["isoGroup"]));
const isoMap = makeMap(data, isoGroupArray, "isoGroup");
const zArray = isoGroupArray.map((d) =>
  d === "" ? 0 : d === 0 ? 0.05 : d === 1 ? 0.25 : d === 2 ? 0.5 : 0.75
);
...
function interpolatePositions(
  { globalIndicesForThisMesh, targetCoords },
  progress
) {
  globalIndicesForThisMesh.forEach((item, i) => {
    animatedCoords[item][0] =
      (1 - progress) * animatedCoords[item][0] +
      progress * targetCoords[globalIndicesForThisMesh[i]][0];
    animatedCoords[item][1] =
      (1 - progress) * animatedCoords[item][1] +
      progress * targetCoords[globalIndicesForThisMesh[i]][1];
    animatedCoords[item][2] =
      (1 - progress) * animatedCoords[item][2] +
      progress * targetCoords[globalIndicesForThisMesh[i]][2];
  });
}
  ...
  */
  ...
  useLayoutEffect(() => {
    if (model === "grid") {
      ...
      const { scratchArray, binTicks } = makeHist(
        data,
        xcol,
        xcolAsc,
        ycol,
      ...
      const { scratchArray, xTickArray, yTickArray, zTickArray } = makeScatter(
        data,
        xcol,
        xcolAsc,
        ycol,
    ...
    */
    ...
    globalIndicesForThisMesh.forEach((item, i) => {
      if (!clickedItems.includes(item)) {
        // so we don't recolor the clicked point
        /*
      ...
      if (!multiClick) {
        ...
        Object.keys(glyphMap).forEach((item, i) => {
          ...
          glyphMap[item].forEach((globalIndex, j) => {
            ...
            } else if (globalInstanceId === globalIndex) {
              // loop iteration item IS ALSO the item we just clicked
              if (!clickedItems.includes(globalInstanceId)) {
                // if this item not already clicked, highlight it
                colorSubstrate.set(highlightColor);
                mesh.setColorAt(j, colorSubstrate);
          ...
          glyphMap[item].forEach((globalIndex, j) => {
            ...
            if (globalInstanceId !== globalIndex) {
              ...
              if (!clickedItems.includes(globalIndex)) {
                ...
                // but if it is clicked, set to highlight color
                colorSubstrate.set(highlightColor);
                mesh.setColorAt(j, colorSubstrate);
              }
            } else if (globalInstanceId === globalIndex) {
              ...
              if (!clickedItems.includes(globalInstanceId)) {
                ...
                mesh.setColorAt(j, colorSubstrate);
                setClickedItems((clickedItems) =>
                  clickedItems.filter((d) => d !== globalInstanceId)
                );
                if (raisedItem !== null && raisedItem === globalInstanceId) {
                  setRaisedItem(null);
                }
  ...
  if (glyph === "box") {
    ...
    return (
      <instancedMesh
        ref={meshRef}
        args={[null, null, glyphMap[glyphGroup].length]}
        onClick={handleClick}
        name={glyphGroup}
      >
...
export function getDetailImageString(texture, backprintImage, i) {
  ...
  const sb = data["sb"][i];
  const detailFolder = texture
    ? "texture/"
    : backprintImage
    ? "backprints_detail/bp"
    : sb === "1" || sb === "-1"
    ? "samplebooks_2048/"
    : "packages_2048/";
...
export default function App({ setPage }) {
  ...
  const [filteredThicknessFrequencies, setFilteredThicknessFrequencies] =
    useState(thicknessValCounts);
  const [filteredColorFrequencies, setFilteredColorFrequencies] =
    useState(colorValCounts);
  const [filteredTextureFrequencies, setFilteredTextureFrequencies] =
    useState(textureValCounts);
  const [filteredGlossFrequencies, setFilteredGlossFrequencies] =
    useState(glossValCounts);
  const [filteredManFrequencies, setFilteredManFrequencies] =
    useState(manValCounts);
  const [filteredBranFrequencies, setFilteredBranFrequencies] =
    useState(branValCounts);
  const [filteredCollFrequencies, setFilteredCollFrequencies] =
    useState(collValCounts);

  const [filteredProcessingFrequencies, setFilteredProcessingFrequencies] =
    useState(processingValCounts);
  const [filteredBackpFrequencies, setFilteredBackpFrequencies] =
    useState(backpValCounts);
  const [filteredSurfFrequencies, setFilteredSurfFrequencies] =
    useState(surfValCounts);
  const [filteredResinFrequencies, setFilteredResinFrequencies] =
    useState(resinValCounts);
  const [filteredTonerFrequencies, setFilteredTonerFrequencies] =
    useState(tonerValCounts);
  const [filteredPostcardFrequencies, setFilteredPostcardFrequencies] =
    useState(postcardValCounts);
  const [filteredCircaFrequencies, setFilteredCircaFrequencies] =
    useState(circaValCounts);
  const [filteredSbidFrequencies, setFilteredSbidFrequencies] =
    useState(sbidValCounts);
  const [filteredDimsFrequencies, setFilteredDimsFrequencies] =
    useState(dimsValCounts);
  ...
  useLayoutEffect(() => {
    const workingIdxList =
      filterIdxList.length === 0 && filter
        ? []
        : filterIdxList.length === 0 && !filter
        ? data["catalog"].map((_, i) => i)
        : filterIdxList;

    const filteredThicknessValCounts = valueCounts(
      data["thicknessWord"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredColorValCounts = valueCounts(
      data["colorWord"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredTextureValCounts = valueCounts(
      data["textureWord"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredGlossValCounts = valueCounts(
      data["glossWord"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredManValCounts = valueCounts(
      data["man"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredBranValCounts = valueCounts(
      data["bran"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredCollValCounts = valueCounts(
      data["sb"].filter((_, i) => workingIdxList.includes(i))
    );

    const filteredProcessingValCounts = valueCounts(
      data["processing"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredBackpValCounts = valueCounts(
      data["backp"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredSurfValCounts = valueCounts(
      data["surf"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredResinValCounts = valueCounts(
      data["resin"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredTonerValCounts = valueCounts(
      data["toner"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredPostcardValCounts = valueCounts(
      data["postcard"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredCircaValCounts = valueCounts(
      data["circa"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredSbidValCounts = valueCounts(
      data["sbid"].filter((_, i) => workingIdxList.includes(i))
    );
    const filteredDimsValCounts = valueCounts(
      data["dims"].filter((_, i) => workingIdxList.includes(i))
    );
    ...
    const filteredYears = data["year"].filter((_, i) =>
      workingIdxList.includes(i)
    );
    const filteredThicknesses = data["thickness"].filter((_, i) =>
      workingIdxList.includes(i)
    );
    const filteredColors = data["dmin"].filter((_, i) =>
      workingIdxList.includes(i)
    );
    const filteredTones = data["dmax"].filter((_, i) =>
      workingIdxList.includes(i)
    );
    const filteredRoughnesses = data["roughness"].filter((_, i) =>
      workingIdxList.includes(i)
    );
    const filteredGlosses = data["gloss"].filter((_, i) =>
      workingIdxList.includes(i)
    );
    const filteredUvs = data["auc"].filter((_, i) =>
      workingIdxList.includes(i)
    );
    const filteredExps = data["expressiveness"].filter((_, i) =>
      workingIdxList.includes(i)
    );
    ...
    if (filteredYearsMin == null || filteredYearsMax == null) {
      ...
      setYearSlideMarks([
        { value: filteredYearsMin, label: filteredYearsMin.toString() },
        { value: filteredYearsMax, label: filteredYearsMax.toString() },
      ]);
    }
    ...
    if (filteredThicknessesMin == null || filteredThicknessesMax == null) {
      ...
      setThicknessSlideMarks([
        {
          value: filteredThicknessesMin,
          label: filteredThicknessesMin.toString(),
        },
        {
          value: filteredThicknessesMax,
          label: filteredThicknessesMax.toString(),
        },
      ]);
    }
    ...
    if (filteredColorsMin == null || filteredColorsMax == null) {
      ...
      setColorSlideMarks([
        { value: filteredColorsMin, label: filteredColorsMin.toString() },
        { value: filteredColorsMax, label: filteredColorsMax.toString() },
      ]);
    }
    ...
    if (filteredTonesMin == null || filteredTonesMax == null) {
      ...
      setToneSlideMarks([
        { value: filteredTonesMin, label: filteredTonesMin.toString() },
        { value: filteredTonesMax, label: filteredTonesMax.toString() },
      ]);
    }
    ...
    if (filteredRoughnessesMin == null || filteredRoughnessesMax == null) {
      ...
      setRoughnessSlideMarks([
        {
          value: filteredRoughnessesMin,
          label: filteredRoughnessesMin.toString(),
        },
        {
          value: filteredRoughnessesMax,
          label: filteredRoughnessesMax.toString(),
        },
      ]);
    }
    ...
    if (filteredGlossesMin == null || filteredGlossesMax == null) {
      ...
      setGlossSlideMarks([
        { value: filteredGlossesMin, label: filteredGlossesMin.toString() },
        { value: filteredGlossesMax, label: filteredGlossesMax.toString() },
      ]);
    }
    ...
    if (filteredUvsMin == null || filteredUvsMax == null) {
      ...
      setUvSlideMarks([
        { value: filteredUvsMin, label: filteredUvsMin.toString() },
        { value: filteredUvsMax, label: filteredUvsMax.toString() },
      ]);
    }
    ...
    if (filteredExpsMin == null || filteredExpsMax == null) {
      ...
      setExpSlideMarks([
        { value: filteredExpsMin, label: filteredExpsMin.toString() },
        { value: filteredExpsMax, label: filteredExpsMax.toString() },
      ]);
    }
  }, [filterIdxList]);
  ...
  const handleDetailScreenNav = (e) => {
    ...
    const clickedItemsPositionIndex = clickedItems.findIndex(
      (d) => d === detailImageIndex
    );
    ...
    if (
      newClickedItemsPositionIndex >= 0 &&
      newClickedItemsPositionIndex <= clickedItems.length - 1
    ) {
      ...
      const newDetailImageString = getDetailImageString(
        texture,
        backprintImage,
        newDetailImageIndex
      );
  ...
  const handlePackageImages = (e) => {
    ...
    const packageImageArray = suffsArr.map(
      (suff) => returnDomain() + "packages_2048/" + catalog + suff + ".jpg"
    );
    ...
    if (
      newPackageImageIndex >= 0 &&
      newPackageImageIndex <= packageImageArray.length - 1
    ) {
  ...
  const handleSliderFilter = (e) => {
    ...
    if (
      yearSlide[0] === yearMin &&
      yearSlide[1] === yearMax &&
      thicknessSlide[0] === thicknessMin &&
      thicknessSlide[1] === thicknessMax &&
      colorSlide[0] === colorMin &&
      colorSlide[1] === colorMax &&
      toneSlide[0] === toneMin &&
      toneSlide[1] === toneMax &&
      roughnessSlide[0] === roughnessMin &&
      roughnessSlide[1] === roughnessMax &&
      glossSlide[0] === glossMin &&
      glossSlide[1] === glossMax &&
      uvSlide[0] === uvMin &&
      uvSlide[1] === uvMax &&
      expSlide[0] === expMin &&
      expSlide[1] === expMax &&
  ...
  const handleFilter = (e) => {
    ...
    if (alreadyInFilterList) {
      newFilterList[dataCat] = newFilterList[dataCat].filter(
        (d) => d !== dataVal
      ); // remove from filter
    } else {
    ...
    if (
      Object.values(newFilterList).every((d) => d.length === 0) &&
      yearSlide[0] === yearMin &&
      yearSlide[1] === yearMax &&
      thicknessSlide[0] === thicknessMin &&
      thicknessSlide[1] === thicknessMax &&
      ...
      function detailNav(e) {
        ...
        if (newClickedItemsPositionIndex !== undefined) {
          if (
            ...
            const newDetailImageIndex =
              clickedItems[newClickedItemsPositionIndex];
            setDetailImageIndex(newDetailImageIndex);
            setPackageImageIndex(0);
        ...
        if (
          newPackageImageIndex !== undefined &&
          detailImageStringState.includes("packages_")
        ) {
          const suffs = data["suffs"][detailImageIndex];
          const suffsArr = suffs.split("|");
          suffsArr.unshift("");
          ...
          const packageImageArray = suffsArr.map(
            (suff) =>
              returnDomain() + "packages_2048/" + catalog + suff + ".jpg"
          );
  ...
  return (
    <div id="app">
      ...
      />
      <div
        className={isDragging ? "noPointerEvents controls" : "controls"}
        id="multiClick"
      >
        <div id="panelStructure">
          <button
            ...
            onClick={() =>
              setPanelLayout((panelLayout) => (panelLayout % 3) + 1)
            }
          >
            grid_view
          </button>
        ...
        <div id="panelFill">
          <button
            title="add paper color to background"
            className={
              backgroundColor ? "material-icons active" : "material-icons"
            }
            onClick={() => {
              setBackgroundColor(!backgroundColor);
              texture && setTexture(false);
              packageImage && setPackageImage(false);
              svgRadar && setSvgRadar(false);
              backprintImage && setBackprintImage(false);
            }}
          >
            format_color_fill
          </button>
          <button
            title="add paper texture to background"
            className={texture ? "material-icons active" : "material-icons"}
            onClick={() => {
              setTexture(!texture);
              backgroundColor && setBackgroundColor(false);
              packageImage && setPackageImage(false);
              svgRadar && setSvgRadar(false);
              backprintImage && setBackprintImage(false);
            }}
          ...
          <button
            title="add package image to background"
            className={
              packageImage ? "material-icons active" : "material-icons"
            }
            onClick={() => {
          ...
          <button
            title="add backprint image to background"
            className={
              backprintImage ? "material-icons active" : "material-icons"
            }
            onClick={() => {
              setBackprintImage(!backprintImage);
              backgroundColor && setBackgroundColor(false);
              texture && setTexture(false);
              svgRadar && setSvgRadar(false);
              packageImage && setPackageImage(false);
          ...
          <button
            title="add radar glyph to background"
            className={svgRadar ? "material-icons active" : "material-icons"}
            onClick={() => {
              setSvgRadar(!svgRadar);
              backgroundColor && setBackgroundColor(false);
              texture && setTexture(false);
              packageImage && setPackageImage(false);
              backprintImage && setBackprintImage(false);
        ...
        <div id="panelText">
          <button
            title="change text length"
            className="material-icons"
            onClick={() => setTextLength((textLength) => (textLength + 1) % 3)}
          >
            short_text
          </button>
          <button
            ...
            className="material-icons"
            onClick={() =>
              setInfoPanelFontSize(
                (infoPanelFontSize) => (infoPanelFontSize % 3) + 1
              )
            }
          >
            format_size
        ...
        <div id="panelSelection">
          <button
            ...
            className={multiClick ? "material-icons active" : "material-icons"}
            onClick={() => setMultiClick(!multiClick)}
          >
            done_all
          </button>
          <button
          ...
          <Download
            data={data}
            idxList={clickedItems}
            etitle="download selected data as CSV"
          />
          <div
            title="number of selected items"
            className="countPrint"
            id="panelCount"
          >
      ...
      <div
        id="infoPanel"
        className={
          isDragging && panelLayout > 1
            ? "grid noPointerEvents"
            : !isDragging && panelLayout > 1
            ? "grid"
            : isDragging
            ? "list noPointerEvents"
            : "list"
        }
      >
        {clickedItems.map((clickedItem, i) => (
          <PanelItem
            data={data}
            meshList={meshList}
            colorVals={colorVals}
      ...
      >
        ...
        >
          ...
          {glyph === "exp" &&
            expressivenessGroupArray.map((d, i) => {
              return (
                <Glyphs
                  key={i}
          ...
          {model === "scatter" && xTicks && (
            <Text
              position={[
                xTicks[Math.floor(xTicks.length / 2)].pos[0],
                xTicks[Math.floor(xTicks.length / 2)].pos[1] -
                  axisTickFontSize * 2,
          ...
          {model === "scatter" && yTicks && (
            <Text
              position={[
                yTicks[Math.floor(yTicks.length / 2)].pos[0] -
                  axisTickFontSize * 4,
          ...
          {model === "scatter" && zTicks && (
            <Text
              position={[
                zTicks[Math.floor(zTicks.length / 2)].pos[0],
                zTicks[Math.floor(zTicks.length / 2)].pos[1] -
                  axisTickFontSize * 2,
      ...
      <div
        ref={selectionDivRef}
        style={{ position: "absolute", border: "4px dashed #F0F" }}
      ></div>
      <div
        className={isDragging ? "controls noPointerEvents" : "controls"}
        id="cameraReset"
      >
        <button
          id="reset-camera"
        ...
        <button
          ...
          className={boxSelectMode ? "material-icons active" : "material-icons"}
        ...
        <div id="detailScreen">
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              const luxURL =
                "https://lux.collections.yale.edu/api/resolve/objects/lml/" +
                data["catalog"][detailImageIndex];
              window.open(luxURL, "_blank");
            }}
          >
            flare
          </button>
          {data["processing"][detailImageIndex] === "1" && (
            <button
          ...
          <button
            title="close modal"
            className="material-icons detailScreenRemove"
            onClick={(e) => {
              e.stopPropagation();
          ...
          {clickedItems.length > 1 && (
            <button
              title="previous panel item"
              id="previousPanelItem"
              className="material-icons"
              onClick={handleDetailScreenNav}
            >
              keyboard_arrow_left
            </button>
          )}
          ...
          {detailImageStringState.includes("packages_") &&
            data["suffs"][detailImageIndex] !== "" && (
              <button
                title="previous package image"
                id="previousPackageImage"
                className="material-icons"
                onClick={handlePackageImages}
              >
                keyboard_arrow_up
              </button>
            )}
          ...
          <div id="detailScreenInfoBar">
            <p>
              {"#" +
                data["catalog"][detailImageIndex] +
                " " +
                data["man"][detailImageIndex] +
                " " +
                data["bran"][detailImageIndex] +
                " " +
                data["year"][detailImageIndex]}
            </p>
          </div>
        </div>
      )}
      <div id="topControls">
        ...
        >
          <button
            title="box glyph"
            onClick={() => setGlyph("box")}
            className={
              glyph === "box" ? "material-icons active" : "material-icons"
            }
          >
            square
          </button>
          <button
            title="expressiveness glyph"
            onClick={() => setGlyph("exp")}
            className={
              glyph === "exp" ? "material-icons active" : "material-icons"
            }
          >
            aspect_ratio
          </button>
          <button
            title="iso glyph"
            onClick={() => setGlyph("iso")}
            className={
              glyph === "iso" ? "material-icons active" : "material-icons"
            }
          >
            line_weight
          </button>
          <button
            title="radar glyph"
            onClick={() => setGlyph("radar")}
            className={
              glyph === "radar" ? "material-icons active" : "material-icons"
            }
          >
            radar
          </button>
      ...
      <div id="bottomControls">
        ...
        >
          <select
            ...
            title="x-axis"
            disabled={model === "gep" || model === "tmap"}
          >
            {model === "gep" && (
              <option value="cluster plot x">cluster plot x</option>
            )}
            {model === "tmap" && (
              <option value="texture map x">texture map x</option>
            )}
            {model !== "gep" && model !== "tmap" && (
              <>
                ...
                <option value="colorGroupTextureWord">
                  texture description
                </option>
                <option value="colorGroupColorWord">
                ...
                <option value="colorGroupProcessing">
          ...
          {xcolAsc && (
            <button
              className={"material-icons"}
              title="sort x-axis descending"
              onClick={() => setXcolAsc(false)}
              disabled={model === "gep" || model === "tmap"}
            >
              arrow_downward
            </button>
          ...
          {!xcolAsc && (
            <button
              ...
              title="sort x-axis ascending"
              onClick={() => setXcolAsc(true)}
              disabled={model === "gep" || model === "tmap"}
            >
              arrow_upward
            </button>
          ...
          >
            {model === "gep" && (
              <option value="cluster plot y">cluster plot y</option>
            )}
            {model === "tmap" && (
              <option value="texture map y">texture map y</option>
            )}
            {model !== "gep" && model !== "tmap" && (
              <>
                <option value="year">year</option>
          ...
          {ycolAsc && (
            <button
              className={"material-icons"}
              title="sort y-axis descending"
              onClick={() => setYcolAsc(false)}
              disabled={model === "gep" || model === "tmap"}
            >
              arrow_downward
            </button>
          ...
          {!ycolAsc && (
            <button
              className={"material-icons active"}
              title="sort y-axis ascending"
              onClick={() => setYcolAsc(true)}
              disabled={model === "gep" || model === "tmap"}
            >
              arrow_upward
            </button>
          ...
          >
            {model === "gep" && (
              <option value="cluster plot z">no z-axis</option>
            )}
            {model === "tmap" && (
              <option value="texture map z">texture map z</option>
            )}
            {model !== "gep" && model !== "tmap" && (
              <>
          ...
          {zcolAsc && (
            <button
              className={"material-icons"}
              title="sort z-axis descending"
              onClick={() => setZcolAsc(false)}
              disabled={model === "gep" || model === "tmap"}
            >
              arrow_downward
            </button>
          ...
          {!zcolAsc && (
            <button
              className={"material-icons active"}
              title="sort z-axis ascending"
              onClick={() => setZcolAsc(true)}
              disabled={model === "gep" || model === "tmap"}
            >
              arrow_upward
            </button>
          ...
          <button
            title="group color shuffle"
            onClick={() => setGroupColors(makeColorArray())}
            className={"material-icons"}
          >
            shuffle
          </button>
      ...
      >
        <button
          title="grid montage"
          className={
            model === "grid" ? "material-icons active" : "material-icons"
          }
        ...
        <button
          title="histogram"
          className={
            model === "hist" ? "material-icons active" : "material-icons"
          }
          onClick={() => setModel("hist")}
        >
          bar_chart
        </button>
        <button
          title="scatter plot"
          className={
            model === "scatter" ? "material-icons active" : "material-icons"
          }
          onClick={() => setModel("scatter")}
        >
          grain
        </button>
        <button
          title="cluster plot"
          className={
            model === "gep" ? "material-icons active" : "material-icons"
          }
          onClick={() => setModel("gep")}
        >
          bubble_chart
        </button>
        <button
          title="texture map"
          className={
            model === "tmap" ? "material-icons active" : "material-icons"
          }
          onClick={() => setModel("tmap")}
        >
          map
        </button>
      </div>
      ...
      >
        {filterModal !== "closed" && (
          <button
            ...
            className={filter ? "material-icons active" : "material-icons"}
            style={{
        ...
        <Download
          data={data}
          idxList={filterIdxList}
          etitle="download filtered data as CSV"
        />
        <div
          ...
          id="filterCount"
        >
          {!filter ? data["catalog"].length : filterIdxList.length}
        </div>
      </div>
      {filterModal !== "closed" && (
        <div
          id="filterModal"
          className={
            isDragging ? `noPointerEvents ${filterModal}` : filterModal
          }
        >
          {filterModal === "open" && (
            <button
              title="replace selection with filter"
              className="material-icons replaceFilterWithSelection"
              style={{ right: "28vw" }}
          ...
          {filterModal === "open" && (
            <button
              title="add filter to selection"
              className="material-icons addFilterToSelection"
              style={{ right: "28vw" }}
              onClick={handleFilterToSelection}
            >
              queue
          ...
          {filterModal === "open" && (
            <button
              title="expand filter window"
              className="material-icons expandButtons"
              style={{ right: "28vw" }}
              onClick={(e) => {
                e.stopPropagation;
                setFilterModal("expanded");
              }}
            >
              chevron_left
            </button>
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">REFERENCE COLLECTION</p>
            </div>
            {Object.keys(collValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  ...
                  className={
                    filterList["sb"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredCollFrequencies, d)}
                >
                  {d === "0"
                    ? "LML Packages"
                    : d === "-1"
          ...
          <div className="filterCategoryContainer">
            ...
            <div className="sliderContainer">
              <Slider
                ...
                color="primary"
                data-cat="year"
                onChangeCommitted={handleSliderFilter}
                onChange={(e) => setYearSlide(e.target.value)}
                defaultValue={[yearMin, yearMax]}
                valueLabelDisplay="on"
                min={yearMin}
                max={yearMax}
                marks={yearSlideMarks}
              />
            </div>
          </div>
          <div className="filterCategoryContainer">
            <div id="manHead" className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">MANUFACTURER</p>
            </div>
            {!manExpand &&
              Object.keys(manValCounts)
                .sort()
                .slice(0, 20)
                .map((d, i) => (
                  <button
                    ...
                    onClick={handleFilter}
                    className={
                      filterList["man"].includes(d)
                        ? "filterButtonActive"
                        : "filterButton"
                    }
                    style={filterButtonStyle(filteredManFrequencies, d)}
                  >
                    {d}
                  </button>
                ))}
            {!manExpand && (
              <button
                title="expand manufacturer options"
                className="material-icons active filterButton"
                onClick={(e) => {
                  e.stopPropagation;
                  setManExpand(true);
                }}
            ...
            {manExpand && (
              <button
                title="contract manufacturer options"
                className="material-icons active filterButton"
                onClick={(e) => {
                  e.stopPropagation;
                  setManExpand(false);
                  document.getElementById("manHead").scrollIntoView();
                }}
              >
                expand_less
              </button>
          ...
          <div className="filterCategoryContainer">
            <div id="branHead" className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">BRAND</p>
            </div>
            {!branExpand &&
              Object.keys(branValCounts)
                .sort()
                .slice(0, 20)
                .map((d, i) => (
                  <button
                    ...
                    onClick={handleFilter}
                    className={
                      filterList["bran"].includes(d)
                        ? "filterButtonActive"
                        : "filterButton"
                    }
                    style={filterButtonStyle(filteredBranFrequencies, d)}
                  >
                    {d}
                  </button>
                ))}
            ...
            {branExpand && (
              <button
                title="contract brand options"
                className="material-icons active filterButton"
                onClick={(e) => {
                  e.stopPropagation;
                  setBranExpand(false);
                  document.getElementById("branHead").scrollIntoView();
                }}
              >
                expand_less
              </button>
          ...
          <div className="filterCategoryContainer">
            ...
            <div className="sliderContainer">
              <Slider
                ...
                data-cat="thickness"
                onChangeCommitted={handleSliderFilter}
                onChange={(e) => setThicknessSlide(e.target.value)}
                defaultValue={[thicknessMin, thicknessMax]}
                valueLabelDisplay="on"
                min={thicknessMin}
                max={thicknessMax}
                step={0.001}
                marks={thicknessSlideMarks}
              />
            </div>
            {Object.keys(thicknessValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  data-cat="thicknessWord"
                  data-val={d}
                  onClick={handleFilter}
                  className={
                    filterList["thicknessWord"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredThicknessFrequencies, d)}
                >
                  {d}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            ...
            <div className="sliderContainer">
              <Slider
                ...
                data-cat="dmin"
                onChangeCommitted={handleSliderFilter}
                onChange={(e) => setColorSlide(e.target.value)}
                defaultValue={[colorMin, colorMax]}
                valueLabelDisplay="on"
                min={colorMin}
                max={colorMax}
                step={0.01}
                marks={colorSlideMarks}
              />
            </div>
            {Object.keys(colorValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  ...
                  onClick={handleFilter}
                  className={
                    filterList["colorWord"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredColorFrequencies, d)}
                >
                  {d}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">TONE COLOR</p>
            </div>
            <div className="sliderContainer">
              <Slider
                key={sliderKey}
                ...
                data-cat="dmax"
                onChangeCommitted={handleSliderFilter}
                onChange={(e) => setToneSlide(e.target.value)}
                defaultValue={[toneMin, toneMax]}
                valueLabelDisplay="on"
                min={toneMin}
                max={toneMax}
                step={0.01}
                marks={toneSlideMarks}
              />
            </div>
          </div>
          <div className="filterCategoryContainer">
            ...
            <div className="sliderContainer">
              <Slider
                ...
                data-cat="roughness"
                onChangeCommitted={handleSliderFilter}
                onChange={(e) => setRoughnessSlide(e.target.value)}
                defaultValue={[roughnessMin, roughnessMax]}
                valueLabelDisplay="on"
                min={roughnessMin}
                step={0.001}
                max={roughnessMax}
                marks={roughnessSlideMarks}
              />
            </div>
            {Object.keys(textureValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  data-cat="textureWord"
                  data-val={d}
                  onClick={handleFilter}
                  className={
                    filterList["textureWord"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredTextureFrequencies, d)}
                >
                  {d}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            ...
            <div className="sliderContainer">
              <Slider
                ...
                data-cat="gloss"
                onChangeCommitted={handleSliderFilter}
                onChange={(e) => setGlossSlide(e.target.value)}
                defaultValue={[glossMin, glossMax]}
                valueLabelDisplay="on"
                min={glossMin}
                max={glossMax}
                marks={glossSlideMarks}
              />
            </div>
            {Object.keys(glossValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  ...
                  onClick={handleFilter}
                  className={
                    filterList["glossWord"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredGlossFrequencies, d)}
                >
                  {d}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">EXPRESSIVENESS</p>
            </div>
            <div className="sliderContainer">
              <Slider
                key={sliderKey}
                color="primary"
                data-cat="expressiveness"
                onChangeCommitted={handleSliderFilter}
                onChange={(e) => setExpSlide(e.target.value)}
                defaultValue={[expMin, expMax]}
                valueLabelDisplay="on"
                min={expMin}
                max={expMax}
                step={0.01}
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">FLUORESCENCE</p>
            </div>
            <div className="sliderContainer">
              <Slider
                key={sliderKey}
                ...
                data-cat="auc"
                onChangeCommitted={handleSliderFilter}
                onChange={(e) => setUvSlide(e.target.value)}
                defaultValue={[uvMin, uvMax]}
                valueLabelDisplay="on"
                min={uvMin}
                max={uvMax}
                marks={uvSlideMarks}
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">PROCESSING INSTRUCTIONS</p>
            </div>
            {Object.keys(processingValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  ...
                  onClick={handleFilter}
                  className={
                    filterList["processing"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredProcessingFrequencies, d)}
                >
                  {d === "1" ? "True" : "False"}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">BACKPRINT</p>
            </div>
            {Object.keys(backpValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  ...
                  onClick={handleFilter}
                  className={
                    filterList["backp"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredBackpFrequencies, d)}
                >
                  {d}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            <div id="surfHead" className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">SURFACE</p>
            </div>
            {!surfExpand &&
              Object.keys(surfValCounts)
                .sort()
                .slice(0, 20)
                .map((d, i) => (
                  <button
                    ...
                    onClick={handleFilter}
                    className={
                      filterList["surf"].includes(d)
                        ? "filterButtonActive"
                        : "filterButton"
                    }
                    style={filterButtonStyle(filteredSurfFrequencies, d)}
                  >
                    {d}
                  </button>
                ))}
            ...
            {surfExpand && (
              <button
                title="contract surface options"
                className="material-icons active filterButton"
                onClick={(e) => {
                  e.stopPropagation;
                  setSurfExpand(false);
                  document.getElementById("surfHead").scrollIntoView();
                }}
              >
                expand_less
              </button>
          ...
          <div className="filterCategoryContainer">
            <div id="dimsHead" className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">DIMENSIONS</p>
            </div>
            {!dimsExpand &&
              Object.keys(dimsValCounts)
                .sort()
                .slice(0, 20)
                .map((d, i) => (
                  <button
                    ...
                    onClick={handleFilter}
                    className={
                      filterList["dims"].includes(d)
                        ? "filterButtonActive"
                        : "filterButton"
                    }
                    style={filterButtonStyle(filteredDimsFrequencies, d)}
                  >
                    {d}
                  </button>
                ))}
            {!dimsExpand && (
              <button
                title="expand dimensions options"
                className="material-icons active filterButton"
                onClick={(e) => {
                  e.stopPropagation;
                  setDimsExpand(true);
            ...
            {dimsExpand && (
              <button
                title="contract dimensions options"
                className="material-icons active filterButton"
                onClick={(e) => {
                  e.stopPropagation;
                  setDimsExpand(false);
                  document.getElementById("dimsHead").scrollIntoView();
                }}
              >
                expand_less
              </button>
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">RESIN-COATED</p>
            </div>
            {Object.keys(resinValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  ...
                  onClick={handleFilter}
                  className={
                    filterList["resin"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredResinFrequencies, d)}
                >
                  {d === "1" ? "True" : "False"}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">TONER</p>
            </div>
            {Object.keys(tonerValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  ...
                  onClick={handleFilter}
                  className={
                    filterList["toner"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredTonerFrequencies, d)}
                >
                  {d}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">POSTCARD</p>
            </div>
            {Object.keys(postcardValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  ...
                  onClick={handleFilter}
                  className={
                    filterList["postcard"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredPostcardFrequencies, d)}
                >
                  {d === "1" ? "True" : "False"}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            <div className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">APPROXIMATE DATE</p>
            </div>
            {Object.keys(circaValCounts)
              .sort()
              .map((d, i) => (
                <button
                  key={i}
                  ...
                  onClick={handleFilter}
                  className={
                    filterList["circa"].includes(d)
                      ? "filterButtonActive"
                      : "filterButton"
                  }
                  style={filterButtonStyle(filteredCircaFrequencies, d)}
                >
                  {d === "1" ? "True" : "False"}
                </button>
              ))}
          ...
          <div className="filterCategoryContainer">
            <div id="sbidHead" className="filterCategoryHeadingContainer">
              <p className="filterCategoryHeading">SAMPLE BOOK</p>
            </div>
            {!sbidExpand &&
              Object.keys(sbidValCounts)
                .sort()
                .slice(0, 20)
                .map((d, i) => (
                  <button
                    ...
                    onClick={handleFilter}
                    className={
                      filterList["sbid"].includes(d)
                        ? "filterButtonActive"
                        : "filterButton"
                    }
                    style={filterButtonStyle(filteredSbidFrequencies, d)}
                  >
                    {d}
                  </button>
                ))}
            {!sbidExpand && (
              <button
                title="expand sample book options"
                className="material-icons active filterButton"
                onClick={(e) => {
                  e.stopPropagation;
                  setSbidExpand(true);
            ...
            {sbidExpand && (
              <button
                title="contract sample book options"
                className="material-icons active filterButton"
                onClick={(e) => {
                  e.stopPropagation;
                  setSbidExpand(false);
                  document.getElementById("sbidHead").scrollIntoView();
                }}
              >
                expand_less
              </button>
...
function filterButtonStyle(filteredFrequencies, d) {
  ...
  if (Object.keys(filteredFrequencies).includes(d)) {
    style.backgroundColor = `hsl(0,0%,${parseInt(
      100 - filteredFrequencies[d] * 100
    )}%)`;
  } else {
