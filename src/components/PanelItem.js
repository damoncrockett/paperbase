...
import { glyphToMap, getDetailImageString } from "./App";
...
export default function PanelItem({
    ...
    if (!multiClick) {
      ...
      Object.keys(glyphMap).forEach((item, i) => {
        ...
        glyphMap[item].forEach((globalIndex, j) => {
          ...
          } else if (clickedItem === globalIndex) {
      ...
      setClickedItems((clickedItems) =>
        clickedItems.filter((d) => d !== clickedItem)
      );
    }
  ...
  const imgStringTexture =
    returnDomain() + "texture_" + textureThumbSize + "/" + catalog + ".jpg";
  const imgStringBackprint =
    returnDomain() +
    "backprints_detail_" +
    textureThumbSize +
    "/bp" +
    ...
    ".jpg";
  const imgString =
    returnDomain() + imgFolder + "_" + imgThumbSize + "/" + catalog + ".jpg";
  const detailImgString = getDetailImageString(
    texture,
    backprintImage,
    clickedItem
  );
  ...
  const svgSide =
    panelLayout === 3 ? window.innerWidth * 0.036 : window.innerWidth * 0.084;
    ...
    <div
      ...
      style={
        backgroundColor
          ...
          : texture
          ? {
              backgroundImage: `url(${imgStringTexture})`,
          ...
          : backprintImage
          ? {
              backgroundImage: `url(${imgStringBackprint})`,
              backgroundPosition: "center",
            }
          : svgRadar
    ...
    >
      {svgRadar && (
        ...
        >
          ...
          <polygon
            points={polygonPoints(data, clickedItem, svgSide)}
            stroke={"none"}
            fill={"#63aaff"}
            fillOpacity={0.5}
          />
      ...
      <button
        ...
        className="openDetailScreen material-icons"
        onClick={(e) => {
          e.stopPropagation();
          setDetailScreen(true);
          setDetailImageStringState(detailImgString);
          setDetailImageIndex(clickedItem);
      ...
      {textLength > 0 && (
        <div className={svgRadar ? "allText fixedOverlay" : "allText"}>
          {textLength === 2 && (
            <div
              className={
                infoPanelFontSize === 1
            ...
            >
              <p>
                {data["catalog"][clickedItem] === "_"
                  ? "#"
                  : "#" + data["catalog"][clickedItem]}
              </p>
            </div>
          )}
          <div className="titleBar">
            <p
              className={
                infoPanelFontSize === 1 && (svgRadar || packageImage)
                  ? "titleBarSmall man lightman"
                  : infoPanelFontSize === 1 && !svgRadar && !packageImage
                  ? "titleBarSmall man"
                  : infoPanelFontSize === 2 && (svgRadar || packageImage)
                  ? "titleBarMid man lightman"
                  : infoPanelFontSize === 2 && !svgRadar && !packageImage
                  ? "titleBarMid man"
                  : svgRadar || packageImage
                  ? "man lightman"
                  : "man"
              }
            >
              {data["man"][clickedItem]}
            </p>
            <p
              className={
                infoPanelFontSize === 1
            ...
            >
              {data["bran"][clickedItem] === "_"
                ? ""
                : data["bran"][clickedItem]}
            </p>
            <p
              className={
                infoPanelFontSize === 1
            ...
            >
              {isNaN(data["year"][clickedItem])
                ? ""
                : data["year"][clickedItem]}
            </p>
          </div>
          {textLength === 2 && (
            <div className="infoBar">
              {writeInfoArray(clickedItem).map((d, i) => (
                <p
                  className={
                    infoPanelFontSize === 1
                      ? "boxWordSmall"
